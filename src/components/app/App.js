...
const App = () => {
  ...
  const [users, setUsers] = useLocalStorage('users', [
    {
      ...
      id: 1,
      messages: [
        { text: "Quickle come to the meeting room 18, we have a big server issue", time: 1660936340820, timeToShow: '9/18/2022 9:25 pm', author: "Alice" },
        { text: "I am having breakfast right now, can`t you wait for 10 minutes?", time: 1660936340842, timeToShow: '9/18/2022 9:27 pm', author: "me" },
      ],
    },
    {
      name: "Josefina",
      ...
      id: 2,
      messages: [
        { text: "We are losing money!", time: 1660936340821, timeToShow: '09/18/2022 11:25 pm', author: "Josefina" },
        { text: "Ok", time: 1660936340834, timeToShow: '9/18/2022 12:25 am', author: "me" },
      ],
    },
    {
      name: "Velazquez",
      ...
      id: 3,
      messages: [
        { text: "Quickly come to the meeting room", time: 1660936340833, timeToShow: '9/18/2022 3:18 am', author: "Velazquez" },
      ],
    },
    {
      name: "Barerra",
  ...
  const sorted = filteredUsers.sort((a,b) => a.messages[a.messages.length-1].time < b.messages[b.messages.length-1].time ? 1 : -1 );
  ...
  const showMessages = users.filter(it => {return it.name === active}).map(item => {
      ...
      return (
        ...
        <div className='container-right-main-list-left'>
          <li>
            ...
            <div>
              <div className='container-right-main-list-text'>{mess.text}</div>
            </div>
          </li>  
          <div className='container-right-main-list-time'>{mess.timeToShow}</div>
        </div>
        : 
        <div className='container-right-main-list-right'>
          <li>
            <div>
              <div className='container-right-main-list-text-me'>{mess.text}</div>
              <div className='container-right-main-list-time-me'>{mess.timeToShow}</div>
            </div>
          </li> 
        </div> 
        }
  ...
  const addMessage = () => {
    ...
    const old = users.filter(it => {return it.name === active})[0]; // копія активного юзера 
    const oldMessages = old.messages; // копія повідомлень активного юзера
    const newMessages = [...oldMessages, {text: textAreaValue, time: date, timeToShow: showDateAndTime, author: 'me'}] // старі повідомлення юзера + нові
    const newItem = {...old, messages: newMessages}; // оновлений юзер з новими повідомленнями
    const newArray = [...users.slice(0, index), newItem, ...users.slice(index+1)] // оновлений масив з новим юзером
    setUsers(newArray)
  ...
  const createMessageFromAPI = (res) => {
    ...
    const old = users.filter(it => {return it.name === active})[0]; // копія активного юзера 
    const oldMessages = old.messages; // копія повідомлень активного юзера
    const newMessages = [...oldMessages, {text: res, time: date, timeToShow: showDateAndTime, author: active}] // старі повідомлення юзера + нові
    const newItem = {...old, messages: newMessages}; // оновлений юзер з новими повідомленнями
    const newArray = [...users.slice(0, index), newItem, ...users.slice(index+1)] // оновлений масив з новим юзером
    setUsers(newArray)
  }
  ...
  const activeUserPhoto = users.filter(it => {return it.name === active}).map(item => {
    return item.img;
  })
